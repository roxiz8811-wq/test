...
local Window = Fluent:CreateWindow({
    Title = "Chop Your Tree - Auto Chop",
    SubTitle = "TreeSpawn Auto Farm (Locked + Real Chop Animation)",
    TabWidth = 160,
...
local ToggleChop = Tabs.Main:AddToggle("AutoChop", {
    Title = "Auto Chop TreeSpawn (ล็อก + ตีช้าเหมือนจริง)",
    Default = false,
    Callback = function(Value)
        autoChop = Value
        if Value then
            equipAxeSlot1()  -- Equip ครั้งแรก + ถือค้าง
            startChopLoop()
            Fluent:Notify({
                Title = "Auto Chop",
                Content = "เปิดแล้ว – ถือขวานค้าง + ตีช้า ๆ มี animation เหวี่ยงปกติ",
                Duration = 5
            })
        else
            ...
            Fluent:Notify({
                Title = "Auto Chop",
                Content = "ปิดแล้ว – ปลดล็อกตำแหน่ง",
                Duration = 3
            })
        end
    end
...
local ToggleFarm = Tabs.Main:AddToggle("AutoLuckyFarm", {
    ...
    Callback = function(Value)
        ...
        Fluent:Notify({
            ...
            Content = Value and "เปิดแล้ว - กำลังฟาร์ม Lucky Blocks..." or "ปิดแล้ว",
            Duration = 4
        })
    end
})

-- Debug Button (ทดสอบ animation + ตี)
Tabs.Main:AddButton({
    Title = "Debug: TP + ล็อก + ตี 1 ครั้ง",
    Description = "ทดสอบ animation เหวี่ยง + ตัดเข้าไหม",
    Callback = function()
        local tree = getTreeSpawn()
        if tree then
            local tpPos = tree.Position + TP_OFFSET
            local targetCFrame = CFrame.lookAt(tpPos, tree.Position)
            hrp.CFrame = targetCFrame
            lockPosition(targetCFrame)
            if not isHoldingTool() then equipAxeSlot1() end
            swingAxeLikeHuman()
            Fluent:Notify({
                Title = "Test สำเร็จ",
                Content = "TP + ล็อก + ตี 1 ครั้ง (ดู animation เหวี่ยงขวาน + เช็ค coins/XP เพิ่มไหม)",
                Duration = 8
            })
        else
            Fluent:Notify({
                Title = "ไม่พบ TreeSpawn",
                Content = "เช็ค plot ของคุณใน Dex",
                Duration = 6
            })
        end
    end
...
Tabs.Main:AddParagraph({
    Title = "หมายเหตุ",
    Content = "• ไม่ถอดอาวุธก่อน → ถือขวานค้างตลอด\n• ตั้งใจให้ตีช้า (1 วินาที/ครั้ง) เพื่อ animation เหวี่ยงสมจริง + ลดเสี่ยงเตะ\n• ถ้า animation ยังไม่ชัด → เพิ่ม EQUIP_WAIT_AFTER เป็น 0.6\n• ถ้าต้องการเร็วขึ้น → ลด CHOP_COOLDOWN เป็น 0.7-0.8 (แต่ดูไม่ธรรมชาติ)"
})
